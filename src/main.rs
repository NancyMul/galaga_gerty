...
#[tokio::main]
async fn main() { // Change this to return Result<(), String>
...
pub struct GameState {
    pub game_board: HashMap<Cords, char>, // Change the value type from char to Ship
    // Add a field - tick_count: u32
    pub player: Player,
    // Add a field - gamelevel: GameLevel
}

impl GameState {
    pub fn new() -> GameState {
        // Create a variable called game_level set to GameLevel::new(Level::Easy)
        // Create a variable called level_status set to game_level.get_level_status();
        
        GameState {
            game_board: HashMap::new(),
            // Add the tick_count field here and set it to 0
            player: Player::new(), // Pass level_status.1 into Player::new()
            // Add the gamelevel field here and set it to game_level
        }
    }
    ...
    pub fn display_board(&self) {
        ...
        for row in 0..ROWS {
            ...
            for col in 0..COLUMNS {
                ...
                // To display how many lives the player has left,
                // we will use '^' placed on the left side of the board.
                // Add another `if` statement at the top of the chain (change the one comparing the player position to an `else if` statement).
                // This new first `if` statement should check if we're on the first row (`row == ROWS - 1`)
                // and if the column number is less than the number of player lives left (minus one, for the current life in question).
                // Here's the logic for that:
                // `col < (self.player.lives - 1) as u8`
                // The body of the `if` statement should print `self.player.display_char`.

                if self.player.current_position == current_position { // change to Some(current_position)
                    print!("{}", '^'); // change from '^' to self.player.display_char
                } else if let Some(ship) = self.game_board.get(&current_position) {
                    print!("{}", ship); // change from ship to ship.display_char()
                } else {
                    print!(" ");
                }
            }
    ...
    pub fn add_ship(&mut self, cords: Cords, ship: char){
...
pub struct Player {
    ...
    // Add a field called lives with type u8
    pub current_position: Cords, // change to Option<Cords>
    // Add a field called start_position with type Cords
    pub death_timer: Timer, 
    pub key_reader: KeyReader,
}

impl Player {
    pub fn new() -> Self { // Fix: take in lives: u8 as a parameter
        let start_position = Cords(ROWS - 2, COLUMNS / 2);
        Player {
            display_char: '^', 
            lives,
            current_position: Cords(ROWS - 2, COLUMNS / 2), // Change to Some(start_position)
            // Add the field start_position and set it to start_position,
